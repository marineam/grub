  ...
  if (data->flags & GRUB_BIOSDISK_FLAG_LBA)
    {
      ...
      grub_dprintf ("biosdisk", "rw 0x%lx blocks at LBA 0x%llx drive 0x%x\n",
		    (unsigned long) dap->blocks,
		    (unsigned long long) dap->block,
		    data->drive);
	...
	return grub_error (GRUB_ERR_OUT_OF_RANGE,
      ...
      grub_dprintf ("biosdisk",
		    "rw 0x%lx sectors at C/H/S %d/%d/%d drive 0x%x\n",
		    (unsigned long) size, coff, hoff, soff, data->drive);
